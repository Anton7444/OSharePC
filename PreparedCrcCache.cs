...
internal static class PreparedCrcCache
{
    ...
    public static void Begin(IEnumerable<string> files)
    {
        ...
        _ = Task.Run(async () =>
        {
            foreach (var path in paths)
            {
                try
                {
                    await GetCrc32Async(path, CancellationToken.None);
                }
                catch (Exception ex)
                {
    ...
    private static State GetOrStart(string path)
    {
        ...
        lock (Gate)
        {
            ...
            var state = new State
            {
                ...
                Work = Task.Run(() => ComputeAsync(path, size, ticks))
            };
            States[path] = state;
            return state;
        }
    ...
    private static async Task<PreparedFile> ComputeAsync(string path, long expectedSize, long expectedTicks)
    {
        ...
        try
        {
            ...
            while (true)
            {
                var read = await fs.ReadAsync(buffer.AsMemory(0, ReadBufferSize));
                if (read <= 0) break;
