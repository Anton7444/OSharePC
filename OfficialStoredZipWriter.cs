        ...
        foreach (var path in files)
        {
            ...
            try
            {
                await using var fs = new FileStream(
                    ...
                    FileMode.Open,
                    FileAccess.Read,
                    // Keep the source stable while its STORED bytes are streamed so
                    // the CRC and ZIP header cannot describe different file contents.
                    FileShare.Read,
                    TransferBufferSize,
            ...
            if (fileSent != size || !finalInfo.Exists || finalInfo.Length != size ||
                finalInfo.LastWriteTimeUtc != fileInfo.LastWriteTimeUtc)
