...
constexpr int kIdleWidth = 340;
constexpr int kIdleHeight = 340;
constexpr int kDragWidth = 460;
constexpr int kDragHeight = 360;
...
bool DesktopDropHotZone::Create(HWND owner) {
  ...
  if (!PositionHotZone(kIdleWidth, kIdleHeight, false, "idle 340x340")) {
    DestroyWindow(window_handle_);
    window_handle_ = nullptr;
    return false;
  }
...
bool DesktopDropHotZone::BeginDrag() {
  ...
  drag_active_ = true;
  UpdateClickThrough();
  if (!PositionHotZone(kDragWidth, kDragHeight, true, "drag 460x360")) {
    drag_active_ = false;
    UpdateClickThrough();
    return false;
  }
  Log("[DesktopDropHotZone] drag active; expanded to 460x360");
  return true;
}
...
bool DesktopDropHotZone::RestoreIdle() {
  ...
  // expanded to 460x360, so visibility alone is not enough to know that the
  // idle bounds are already correct.
  drag_active_ = false;
  UpdateClickThrough();
  return PositionHotZone(kIdleWidth, kIdleHeight, true, "idle 340x340");
}
